...
let tblClientesAdmin;

document.addEventListener('DOMContentLoaded', function () {
    $(document).ready(function () {
        tblClientesAdmin = $('#tblClientesAdmin').DataTable({
            ajax: {
                url: 'clientes.php',
                dataSrc: 'data'
            },
...
function editar(id) {
    var data = tblClientesAdmin.rows().data().toArray().find(row => row.id == id);
...
function redireccionarAAdminInformacion(id) {
    ...
    var url = 'adminInformacion.php?id=' + id;
